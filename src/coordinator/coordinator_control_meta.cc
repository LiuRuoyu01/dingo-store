...
butil::Status CoordinatorControl::CreateSchema(int64_t tenant_id, std::string schema_name, int64_t& new_schema_id,
  ...
  // schema id
  if (new_schema_id > 0) {
    // br resore schema，continue  use schema_id
    if (new_schema_id == INT64_MAX) {
      DINGO_LOG(FATAL) << "new_schema_id=" << new_schema_id << " key=" << pb::coordinator::IdEpochType::ID_NEXT_SCHEMA
                       << ", FATAL id is used up";
    }
  } else {
...
butil::Status CoordinatorControl::RestoreIndexMeta(int64_t schema_id, int64_t index_id,
  ...
  auto ret1 = ValidateMaxIndexCount();
  if (!ret1.ok()) {
    DINGO_LOG(ERROR) << "restore index exceed max index limit, index_definition:"
                     << table_definition_in.ShortDebugString();
    return ret1;
  }
  ...
  DINGO_LOG(INFO) << "RestoreIndexMeta index_definition : " << table_definition.ShortDebugString();
  ...
  index_name_map_safe_temp_.Get(new_index_check_name, value);
  if (value != 0) {
    DINGO_LOG(INFO) << " RestoreIndexMeta index_name is exist " << table_definition.name();
    return butil::Status(pb::error::Errno::EINDEX_EXISTS,
                         fmt::format("index_name[{}] is exist in get", table_definition.name()));
  }

  if (index_id <= 0) {
    DINGO_LOG(ERROR) << "RestoreIndexMeta index_id is illegal:" << index_id;
    return butil::Status(pb::error::Errno::EILLEGAL_PARAMTETERS, "index_id is illegal");
  }

  int ret = index_map_.Exists(index_id);
  if (ret) {
    DINGO_LOG(ERROR) << "RestoreIndexMeta index_id is already used:" << index_id;
    return butil::Status(pb::error::Errno::EILLEGAL_PARAMTETERS, "index_id is already used by other index");
  }

  // update index_name_map_safe_temp_
  if (index_name_map_safe_temp_.PutIfAbsent(new_index_check_name, index_id) < 0) {
    DINGO_LOG(INFO) << " RestoreIndexMeta index_name" << table_definition.name()
                    << " is exist, when insert index_id=" << index_id;
    return butil::Status(pb::error::Errno::EINDEX_EXISTS,
                         fmt::format("index_name[{}] is exist in put if absent", table_definition.name()));
  }
